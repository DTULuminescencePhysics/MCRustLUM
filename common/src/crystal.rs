    ...
    pub fn new<X: Numeric, Y: Numeric, Z: Numeric>(x: X, y: Y, z: Z, periodic: bool) -> Self {
        ...
        if periodic {
            Self {
                ...
                y,
                z,
                kind: BoundaryCondition::Periodic,
            }
        } else {
            Self {
                x,
...
impl ElectronPlaces {
    pub fn new(traps: Vec<Trap>, holes: Vec<Hole>, bandtails: Vec<Bandtail>) -> Self {
        Self {
            traps,
            holes,
            bandtails,
    ...
    pub fn random_new<X: Numeric, Y: Numeric, Z: Numeric>(
        ...
        let traps = (0..t_no).map(|_| Trap::random(x, y, z)).collect();

        let holes = (0..h_no).map(|_| Hole::random(x, y, z)).collect();

        let bandtails = (0..b_no).map(|_| Bandtail::random(x, y, z)).collect();
...
impl Cube {
    ...
    pub fn trap_trap_distance(&self, p1: usize, p2: usize) -> Float {
        self.boundary.distance(
            &self.places.traps[p1].position,
            &self.places.traps[p2].position,
        )
    }
